...
## D74 — Case rows are criterion-scoped, share cached runs, and carry their own clock
...
## D76 — `ERROR` is the harness's fourth status, and the abstention rate never sees it
...
## D77 — The verifier is a third runner port, a step in the graph, and a recording every gate replays
...
REQ-18a's separate loop, reusing D75's boundary: a `VerifierFailure` enum
