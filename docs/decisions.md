...
## D103 — An unanchorable quote is re-asked once for its verbatim text, and every turn is counted

**Context.** T-89, row 5 of `Path to v2` (D97): P2's second half. Spans
are located by searching the model's verbatim quote (D18), so a quote the
model paraphrased is unanchorable and `build_result` drops the claim. The
loss on record is one word — T-63's tool-fetch run wrote *completed* where
E8's note says *completing*, with 62 verbatim characters after it — and it
cost E8 its only evidence, turning `UNSUBSTANTIATED_ASSERTION` into
`NO_EVIDENCE_RETRIEVED`, a different instruction to the specialist (D12,
D71). T-85 put the figure in `eval/report.md` (D98) and named the fix: a
bounded re-ask for the verbatim text. D98 deferred it because a re-ask
changes the call configuration, and D45 says a changed configuration is a
new measurement of every extraction recording.

A second thing this task has to fix travels under D97's phrase *every turn
counted*. `scripts/run_extraction.py` sums the singular per-note `metrics`,
the direct runner never populates `ExtractionResult.trace`, and
`RecordedExtractionRunner` replays one `CallMetrics` per note. A second
turn on the direct path would therefore reproduce D71's 12.1x undercount in
the one measurement script that was immune, and again in every replayed
determination A6 is quoted from.

**Chosen.**

1. **Python decides to re-ask; the anchorer decides what is admitted.**
   After turn one, `build_result` names the quotes that did not anchor. That
   is a Python boolean over string search, not model output, so Article I
   holds and D62's rule — *the model is never asked whether to retry* —
   holds with it. The re-ask returns the note and the failed quotes and asks
   for each one's verbatim text; the answer is a *new quote the anchorer
   searches*, exactly as turn one's was. `anchor.py` and `spans.py` do not
   change, and nothing rewrites a quote to an overlap it found — D98's
   objection to a fallback locator. A quote still unanchorable after the
   re-ask is dropped as before. The re-ask replaces quote fields at the paths
   Python asked about and nothing else: it cannot add, remove or re-date an
   event or an assertion, because the targets are generated by
   `build_result` and the patch is applied only at those paths.
2. **This partially reverses D18's fourth rejection** — *re-prompt the model
   for a shorter or unique quote on a miss*. D18 refused it because Python
   fixed the whitespace case for free and because a re-prompt made the
   failure path model-dependent. Neither objection reaches this: Python
   cannot fix a paraphrase for free (D98 measured the one on record), and
   the failure path stays Python because the anchorer, not the model, admits
   the new quote. This is not a re-ask for offsets, which D17 and D19 showed
   the model cannot produce.
3. **The bound is `REASK_ROUNDS = 1`**, a named constant in
   `pa_agent/extraction.py` with its justification beside it, in the shape of
   `DEFAULT_MAX_LLM_CALLS` and `MAX_ROWS`. A second answer is verbatim or it
   is not; a third ask with the same information is a loop. It is a
   different axis from `workflow.DEFAULT_MAX_ATTEMPTS`, which retries
   *failures*: this retries a partial success. ADK's `max_llm_calls` is per
   `run_async` invocation, so the per-note ceiling under the ADK runner is
   `(1 + REASK_ROUNDS) × DEFAULT_MAX_LLM_CALLS`, restated in the constant's
   docstring rather than lowered — a remaining-budget scheme that reached
   zero would *disable* enforcement in ADK.
4. **A failed re-ask turn never raises.** Turn one's result stands, the
   result's `reask` block carries the classified reason (`CALL_FAILED`,
   `UNPARSEABLE`, `SCHEMA_INVALID`), and the trace's `termination_reason`
   names it — T-29's shape (D76): a hook that failed says so in the trace it
   produced. The measurement scripts re-run a note whose re-ask failed on
   transport (`CALL_FAILED`), because a transport fault is not a measurement
   of the re-ask, and record `UNPARSEABLE` and `SCHEMA_INVALID` as findings
   (D71: recorded, not retried).
5. **The prompt version names both halves.** `PROMPT_VERSION` moves to
   `pa_agent/extraction.py`, where the instruction lives, and becomes
   `t15-instruction-v1/t89-reask-v1`: the extraction instruction is
   byte-identical (the tests assert it) and the call configuration changed
   (D45, D64), and one identifier must say both. `extraction_agent.py`
   re-exports it so existing imports resolve. Every recording writes it, and
   a test pins each of the three recordings' `prompt_version` to the code's.
   That test is the runnable form of *re-measured* — it is red on the old
   recordings by design, and closing the task means making it green with
   spent calls.
6. **Every turn counted, on every path.** Both live runners return an
   `ExtractionResult` whose singular `metrics` is turn one, unchanged, and
   whose `trace` carries every turn, so `workflow._extract_one` counts them
   as it already counts the ADK's tool round trip. The direct recording gains
   a `trace` per note and its aggregate sums turns through the same rule
   `run_adk_extraction.py` applies. The recorded runner replays the recorded
   trace, re-addressed to the requesting document for a clone (D102), so A6
   and the agentic oracle column count the re-ask. The 4.3x input-token ratio
   D91 warned about moves with the shared denominator, and is re-quoted with
   its delta.
7. **The re-measured recordings are T-89's.** They carry `task: "T-89"`,
   `decision: "D103"` and a `supersedes` field naming the measurement they
   replace (`T-15 (D45)`, `T-63 (D71)`), and the report labels them so. A
   recording produced under this configuration and still stamped `T-15`
   would claim T-15's configuration for a run that did not use it.
8. **A recovered event quote can expose per-field drops** that were never
   asked, because turn one skips an event's BMI, diet and activity quotes
   when the event quote itself fails. They stay dropped, they are not
   targets, and the audit does not count them as unrecovered. Bounded, and
   said here so a later reader does not mistake it for a bug.
9. One fold, on D68's precedent: while `_invoke` is being split, ADK's
   `ValidationError` on the final answer is classified `SCHEMA_INVALID`
   instead of falling into `CALL_FAILED`, which is why
   `test_a_payload_the_schema_rejects_raises` tolerated three reasons.

**The `reask` audit.** Per note: the targets (path, reason, full quote),
the answers, the paths recovered and unrecovered, and the error if any;
`raw` is the payload the result was built from, patched, so replay is exact,
and `raw_first_turn` sits beside it so `--rescore` can re-derive the
recovered set by anchoring both for zero calls. The report's anchoring
section gains re-asked and recovered columns per recording, read from the
per-note blocks and never from a stored aggregate (T-71's rule).

**REQ-56** states the mechanism as a requirement, mapped to
`tests/test_reask.py`. D97's table says *retry*; *re-ask* is the canonical
word from here, because the thing retried is a question and not a failure.

**Rejected — a similarity fallback in `anchor.py`.** D98's argument
verbatim: a match generous enough to absorb *completed/completing* is
generous enough to absorb *not*, and it cannot be built without the locator
rewriting the quote to pass the validator.

**Rejected — changing the first-turn instruction to ask for shorter anchor
phrases.** The thing that failed was a short phrase (D98). It would also
change the half of the configuration this entry keeps fixed, so the three
recordings' fidelity figures would stop being comparable to D45's and D71's.

**Rejected — the re-ask inside `workflow.py`.** The workflow would then
know a runner's payload shape, and a runner's turn count would depend on
which caller ran it; D97 named the runners.

**Rejected — unbounded or iterative re-asking.** A loop keyed off whether
the last answer anchored is a loop keyed off model output (Art. I); one
round keyed off a Python predicate is a step.

**Rejected — raising on a failed re-ask.** It sends a successful extraction
back through the workflow's attempt budget and, on exhaustion, reports
"the system did not look" for a note it read correctly — D90's conflation
built into the fix.

**Rejected — assertions only.** The failure on record is an assertion, but
the mechanism is the same for every quote field, and D98's reversal clause
already names an event drop as the thing that would raise the priority. One
mechanism, one measurement.

**Cost.** One more model call on any note with an unanchorable quote, and a
re-measurement of all three extraction recordings plus whatever the moved
spans force downstream — the verifier recording is keyed by the sliced
quotes, so a span whose text moved is a claim digest that moved.

**Reverses if:** a measured round recovers nothing across all three
recordings while paying the turn (the constant then goes to zero and the
