...
- **The ground truth was authored by the agent building the system it grades** (D19, D42). Structural mitigations are in place — the manifests are written from the bundles before the notes are synthesized, and the system under test never reads them — and a perfect score still means only that the approach does not obviously fail.
- **No pass that would retire that caveat is scheduled** (D92). T-78 was the owner's adjudication of every label and manifest; the ratification programme it belonged to was deleted, so every figure here is measured against labels the system's own author wrote, and will stay that way unless someone who did not author them adjudicates them.
