"""T-17 — Article V's verifier is blind, and a rejection is an abstention (D78).
...
def test_the_payload_carries_the_claim_and_nothing_else(tree) -> None:
    ...
    assert sorted(payload) == ["criterion", "quotes", "verdict"], (
        ...
        "presence date-bound every claim digest (D78)"
    )
...
# --------------------------------------------------------------------------
# Faults: REQ-18a's loop, mapped at the workflow boundary (D76's shape)
# --------------------------------------------------------------------------
...
def test_a_response_off_schema_is_terminal(tree) -> None:
    ...
    assert caught.value.reason is VerifierFailure.SCHEMA_INVALID, (
        ...
        "threshold waiting to be invented (D78)"
    )
...
def recording() -> dict:
    assert VERIFIER_RESULTS.exists(), (
        ...
        "and commit eval/verifier/results.json (T-17, D78)"
    )
