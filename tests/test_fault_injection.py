"""T-29 — fault injection and the no-silent-failure audit (D75).
...
def test_a_raising_model_call_errors_after_the_budget_and_the_count_is_checked(
    ...
    assert [r.criterion_id for r in caught.value.results] == list(
        EXTRACTION_CRITERIA
    ), "an extraction failure errors the extraction-consuming criteria (D75)"
...
#: what keeps this list from absorbing whatever is convenient (D75). Keyed by
